...
const caValidation = [
    ...
    check('TenCa').isLength({ max: 5 }).withMessage('Tên ca tối đa 5 ký tự').notEmpty().withMessage('Tên ca là bắt buộc'),
    check('GioBatDau').matches(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9]$/).withMessage('Giờ bắt đầu phải có định dạng HH:mm:ss').notEmpty().withMessage('Giờ bắt đầu là bắt buộc'),
    check('GioKetThuc').matches(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9]$/).withMessage('Giờ kết thúc phải có định dạng HH:mm:ss').notEmpty().withMessage('Giờ kết thúc là bắt buộc')
];
...
router.get('/ca/search', searchValidation, validate, caController.searchCa);
